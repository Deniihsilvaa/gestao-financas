...
function Contas() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const filteredData = filterByDateAndSituacao(data, selectedStartDate, selectedEndDate, selectedSituacao);
  ...
  const filterByDateAndSituacao = (data, startDate, endDate, situacao) => {
    return data.filter(item => {
      ...
      const isInDateRange = (!startDate || itemDate >= new Date(startDate)) && (!endDate || itemDate <= new Date(endDate));
  ...
  const groupByCategoryAndMonth = (data) => {
    const grouped = {};
    data.forEach((item) => {
      const month = new Date(item.data_transacao).toLocaleString("default", { month: "short" });
  ...
  const getReceitaDespesa = (data) => {
    const receita = data.filter(item => item.tipo_registro === "Entrada").reduce((sum, item) => sum + item.valor, 0);
    const despesa = data.filter(item => item.tipo_registro === "Saída").reduce((sum, item) => sum + item.valor, 0);
