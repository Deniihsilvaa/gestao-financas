...
function Contas() {
  ...
  // Funções para filtrar os dados para cada gráfico
  ...
  const filtrarSaidas = () => filtrarPorTipoRegistro(data, "Saída");

  // Função para filtrar os dados de "Entrada" para o gráfico de receita/despesa
  const filtrarEntradas = () => filtrarPorTipoRegistro(data, "Entrada");

  // Função para filtrar dados por categoria para o gráfico de colunas
  const filtrarPorCategoria = () => {
    return data.filter((item) => item.tipo_categoria);
  };
  ...
  return (
    <div className="p-6 text-white rounded-lg shadow-xl bg-gradient-to-r from-blue-900 to-black">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        <div className="card">
          <GraficoPizza data={filtrarSaidas()} />
        </div>
        <div className="card">
          <GraficoReceitaDespesas receita={receita} despesa={despesa} />
        </div>
        <div className="card">
          <GraficoColunas data={filtrarPorCategoria()} />
        </div>
        <div className="card">
