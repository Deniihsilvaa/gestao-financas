...
function GraficoPizza({ data }) {
  ...
  const filteredData = data
    .filter(item => item.tipo_registro === "Saída" && item.meses[currentMonth]) // Filtra por "Saída" e mês atual
    .map(item => ({
      tipo_categoria: item.tipo_categoria,
      valor: item.meses[currentMonth].reduce((sum, record) => sum + record.valor, 0), // Soma os valores do mês
    }));
