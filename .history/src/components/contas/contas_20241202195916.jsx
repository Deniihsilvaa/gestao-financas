...
function Contas() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Filtrando dados pela data, se uma data estiver selecionada
      const filteredData = filterByDate(data, selectedDate);

      const groupedData = groupByCategoryAndMonth(filteredData);
      const currentMonth = getCurrentMonthData(filteredData);
      const yearData = getYearlyData(filteredData);
      const situacao = groupBySituacao(filteredData);
      ...
      const { receita, despesa } = getReceitaDespesaMesAtual(filteredData);
      setReceita(receita);
      setDespesa(despesa);
    }
  ...
  const filterByDate = (data, selectedDate) => {
    if (!selectedDate) return data; // Se nenhuma data for selecionada, não filtra

    const startDate = new Date(selectedDate);
    const endDate = new Date(selectedDate);
    endDate.setDate(endDate.getDate() + 1); // Adiciona 1 dia para filtrar até o final do dia
