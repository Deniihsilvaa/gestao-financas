...
function TabelaCategoria({ data }) {
  ...
  const rowExpansionTemplate = (rowData) => {
    return (
      <div className="p-3">
        <div><strong>Descrição:</strong> {rowData.descricao}</div>
        <div><strong>Data da Transação:</strong> {rowData.data_transacao}</div>
        <div><strong>Tipo de Registro:</strong> {rowData.tipo_registro}</div>
        <div><strong>Conta Bancária:</strong> {rowData.conta_bancaria}</div>
        <div><strong>Situação:</strong> {rowData.situacao}</div>
      </div>
    );
  };
